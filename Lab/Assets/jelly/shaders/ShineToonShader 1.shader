Shader "Custom/ShuoenShader"
{
	...
	SubShader {
		...
		void vert(inout appdata_full v) {
             ...
             v.vertex.y += sin(_Time.y * 4 + v.vertex.x * 2) * 0.2 * 0.19;
			 //v.vertex.y += sin(_Time.y * 4 + v.vertex.x *8) * 0.04 - cos(_Time.y * 4 + v.vertex.x * 8) * 0.05  ;
			 v.vertex.x += v.normal * clamp(sin(_Time.y) * 0.05, 0, 1);
         }
		struct Input {
			float2 uv_MainTex;
		};
		...
		{		
			...
			if (diffAlbedo.r < 0.5){
				diffAlbedo.r = 0.3;
			}else if (diffAlbedo.r < 0.6){
				diffAlbedo.r = 0.6;
			}else if (diffAlbedo.r >0.9){
				diffAlbedo.r = 0.8;
			}
			...
			/**
			if (c.r < 0.8 ){
				c = fixed4((abs(sin(_Time.y* 0.5)*0.6) + 0.3), 0.7 -(abs(sin(_Time.y* 0.52)*0.4)), 0.9+(abs(sin(_Time.y* 0.52)*0.1)), 1.0);
